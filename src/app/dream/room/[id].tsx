...
import { dreamJoinRoom, joinRoom } from "@/app/lib/function";
...
export default function Room({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
      ...
      const config = {
        ...
        leftScreen: {
          ...
          actionButton: {
            ...
            href: "https://catalyst.jerometanofficial.com/dream/", // action button href
          },
        },
    ...
    const loadRoomInfo = async () => {
      const roomInfo = await dreamJoinRoom(params.id);
      const parsedRoomInfo = await JSON.parse(roomInfo);
      if (parsedRoomInfo.status !== 200) {
        return alert("Failed to fetch room info");
      }
